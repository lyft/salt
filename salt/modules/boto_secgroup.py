...
def _add_group_object_context(group_id, group_object):
    ...
    if __context__.get('_group_object') is None:
        # if _get_group_id does not exist, create a dict to hold group ids
        __context__['_group_object'] = {}
    # push the group id, name and vpc_id into context
    context_entry = {group_object.id: group_object}
    __context__['_group_object'].update(context_entry)


def _get_group_object_context(group_id=None, name=None, vpc_id=None):
    if __context__.get('_group_object') is None:
        # if _get_group_id does not exist, create a dict to hold group ids
        __context__['_group_object'] = {}
    group_object = None
    if group_id:
        ...
        if __context__['_group_object'].get(group_id):
            logging.debug('found group id {0} in context'.format(group_id))
            group_object = __context__['_group_object'][group_id]
        return group_object
    elif name:
        ...
        for _group_id in __context__['_group_object']:
            if (__context__['_group_object'][_group_id].name == name
                and __context__['_group_object'][_group_id].vpc_id == vpc_id):
                group_object = __context__['_group_object'][_group_id]
...
def _get_group_id(conn, name, vpc_id=None):
    ...
    group_object = _get_group_object_context(name=name, vpc_id=vpc_id)
    if group_object:
    ...
    if vpc_id is None:
        ...
        for group in filtered_groups:
            ...
            if group.vpc_id is None:
                _add_group_object_context(filtered_groups[0].id, filtered_groups[0])
...
def _exists_group_id(conn, group_id):
    ...
    group_id_search = _get_group_object_context(group_id=group_id)
    if group_id_search:
        return True
...
def get_config(name=None, group_id=None, vpc_id=None, region=None, key=None,
    ...
    group_object = _get_group_id(conn, name, vpc_id)
    ...
    try:
        ...
        if name:
            group_id = _get_group_id(conn, name, vpc_id)
        if group_id:
            group_object = _get_group_object_context(group_id=group_id)
            if group_object:
                sg = group_object
            else:
                group_filter = {'group-id': group_id}
